...

{
    ...
    while IFS=$'\t' read -r db_id agent_id name; do
        ...
        cash_value="$(jq -r --arg a "$agent_id" '.wallets[] | select(.agent_id == $a) | .cash // "0"' "$WALLETS_JSON" | head -n1)"
        wallet_size="${wallet_size:-0}"
        cash_value="${cash_value:-0}"
        mtm_positions="$(echo "$positions_json" | jq '[.positions[].current_value // 0] | add // 0')"
        unrealized="$(echo "$positions_json" | jq '[.positions[].unrealized_pnl // 0] | add // 0')"
        start="$(jq -n --arg ws "$wallet_size" '($ws|tonumber? // 0)')"
        equity="$(jq -n --arg c "$cash_value" --argjson m "$mtm_positions" '($c|tonumber? // 0) + $m')"
